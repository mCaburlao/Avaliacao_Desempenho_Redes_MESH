...
O roteamento em redes mesh é crítico pois determina latência, consumo energético, confiabilidade e escalabilidade da rede. Os protocolos de roteamento em redes mesh classificam-se em três categorias principais: proativos, reativos e híbridos \cite{wireless_benyamina_2012}. Cada paradigma apresenta trade-offs distintos em termos de overhead de controle, latência de descoberta de rotas e adaptabilidade a mudanças topológicas.
...
O presente trabalho endereça esta lacuna por meio de uma análise comparativa controlada de AODV (reativo) e OLSR (proativo) em escalas que refletem aplicações reais de cidades inteligentes. Utiliza-se simulação em MeshSim (baseada em NS-3) para garantir reprodutibilidade absoluta e coleta precisa de métricas.
...
A hipótese de trabalho é que existe regime de escala em que AODV supera OLSR significativamente em overhead de roteamento e latência, enquanto OLSR preserva taxa de entrega superior em redes pequenas onde o overhead proativo é tolerável. Espera-se identificar transição entre estes regimes com precisão estatística ≥95\%, permitindo recomendação prática para engenheiros que selecionam protocolo em função de escala esperada da rede.
